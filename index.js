// 1. Filter smartphones released before 2020 and with a camera resolution less than 12MP
const smartphones = [
  ...
  const filteredSmartphones = smartphones.filter(phone => !(phone.year < 2020 && phone.cameraResolution < 12));
  console.log(filteredSmartphones);
  
  
  // 2. Filter books published before 2005 and with more than 400 pages
  const books = [
  ...
  // 3. Filter students with age < 18 and GPA < 3.5
  const students = [
  ...
  // 4. Filter employees with salary < 50000 and hired before 2010
  const employees = [
  ...
  // 5. Filter products with price < 100 and not in stock
  const products = [
  ...
  // 6. Filter movies released before 2010, rating < 8.0, and not in "Action"
  const movies = [
  ...
  // 7. Filter apartments with price < 1500, not in a safe neighbourhood, and <2 bedrooms
  const apartments = [
  ...
  const filteredApartments = apartments.filter(apartment => !(apartment.price < 1500 && !apartment.safeNeighbourhood && apartment.bedrooms < 2));
  console.log(filteredApartments);
  
  
  // 8. Filter products with price < 50, not available online, and not in "Electronics"
  const products2 = [
  ...
  const filteredProducts2 = products2.filter(product => !(product.price < 50 && !product.onlineAvailability && product.category !== "Electronics"));
  console.log(filteredProducts2);
  
  
  // 9. Filter job applicants with <3 years experience, not proficient in English, and no relevant degree
  const jobApplicants = [
  ...
  // 10. Filter restaurants with <4 stars, not serving vegetarian, and outside the city
  const restaurants = [
  ...
  // 11. Filter books published before 2000, rating <7.5, and not in "Mystery" or "Thriller"
  const books2 = [
  ...
  const filteredBooks2 = books2.filter(book => !(book.year < 2000 && book.rating < 7.5 && !["Mystery", "Thriller"].includes(book.genre)));
  console.log(filteredBooks2);
  
  
  // 12. Filter employees with <5 years experience, not proficient in English, and not in "Engineering" or "Marketing"
  const employees2 = [
  ...
  const filteredEmployees2 = employees2.filter(employee => !(employee.experienceYears < 5 && !employee.englishProficiency && !["Engineering", "Marketing"].includes(employee.department)));
